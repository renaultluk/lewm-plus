...
bash scripts/superpod/train_lewm.sh data=pusht_h5 trainer.max_epochs=100
...
- **File not found / bad interpreter**: run the script with `bash scripts/superpod/hello_superpod.sh` (recommended).
...
to `scripts/superpod/superpod.env`. After that, job scripts read this file
...
```bash
bash scripts/superpod/train_lewm.sh data=pusht_h5 trainer.max_epochs=100 output_model_name=pusht_h5_replicate
```
