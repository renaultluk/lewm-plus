...
```bash
cd /project/<GROUP>/lewm-plus
ls docs/scripts/superpod/superpod.env
```
...
```bash
sbatch docs/scripts/superpod/hello_superpod.sh
```
...
- **File not found / bad interpreter**: make sure the script is executable or
  submit with `bash docs/scripts/superpod/hello_superpod.sh` (recommended).
...
Once `hello_superpod.sh` completes successfully, move on to real jobs.
