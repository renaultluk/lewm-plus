...
All scripts below live in `scripts/superpod/` and should be copied to
...
sbatch scripts/superpod/train_lewm.sh data=pusht_h5 trainer.max_epochs=100
...
```bash
cd /project/<GROUP>/lewm-plus
ls scripts/superpod/superpod.env
```
...
```bash
sbatch scripts/superpod/hello_superpod.sh
```
...
- **File not found / bad interpreter**: make sure the script is executable or
  submit with `bash scripts/superpod/hello_superpod.sh` (recommended).
...
```bash
sbatch scripts/superpod/hello_superpod_gpu.sh
```
...
to `scripts/superpod/superpod.env` and updates the Slurm directives in the
...
```bash
sbatch scripts/superpod/train_lewm.sh data=pusht_h5 trainer.max_epochs=100 output_model_name=pusht_h5_replicate
```
...
```bash
bash scripts/superpod/interactive_gpu.sh
